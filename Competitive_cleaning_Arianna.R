...
my_data <- read_excel("C:/Users/Alan.Garcia/Downloads/Arianna_competitive.xls",
                      sheet = "(P) - TGRPS LIBERTY_")
...
# aux_b <- (copetitive$enhanced_category %in% c("Residence","Mobility") ) & (copetitive$Brand_Change %in% institutional)
# copetitive$enhanced_category[aux_b] <- "Institutional"
# sum(aux_b)

#a <- copetitive %>% distinct(Advertiser, Brand_Change, enhanced_category, DUPLICATED)
j <- copetitive %>% distinct(Advertiser,Brand,Brand_Context_Variables, Brand_Change,Category,
                             enhanced_category, DUPLICATED)

write_xlsx(copetitive,"C:/Users/Alan.Garcia/OneDrive - OneWorkplace/Documentos/Trabajos MyOMC/Competitivo/Telecom_Competitive_Arianna.xlsx")
